  ...
  */
    ...
    let
      packagesWithPathInner =
        ...
        if result.success then
          ...
          [ ];
    in
    packagesWithPathInner rootPath mypkgs;
  ...
  packagesWithUpdateScript =
    ...
    let
      ...
      pathContent = lib.attrByPath prefix null mypkgs;
    in
  ...
  packageByName =
    path: _pkgs:
    let
      package = lib.attrByPath (lib.splitString "." path) null mypkgs;
    in
  ...
  packages =
    if package != null then
      [ (packageByName package mypkgs) ]
    else if predicate != null then
      packagesWithUpdateScriptMatchingPredicate predicate mypkgs
    else if maintainer != null then
      packagesWithUpdateScriptAndMaintainer maintainer mypkgs
    else if path != null then
      packagesWithUpdateScript path mypkgs
    else
...
pkgs.stdenv.mkDerivation {
  ...
  shellHook = ''
    ...
    exec ${pkgs.python3.interpreter} ${updatePyScript} ${builtins.concatStringsSep " " args}
  '';
}
